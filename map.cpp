...
int main() {
	...
	core::vector3df sonicPos(0, 0.5, 0);
	...
	while(device->run()){
		...
		if(receiver.IsKeyDown(irr::KEY_KEY_Z))
			sonicPos.set(sonicPos.X + 0.5, sonicPos.Y, sonicPos.Z);
		else if(receiver.IsKeyDown(irr::KEY_KEY_S))
			sonicPos.set(sonicPos.X - 0.5, sonicPos.Y, sonicPos.Z);
		if(receiver.IsKeyDown(irr::KEY_KEY_Q))
			sonicPos.set(sonicPos.X, sonicPos.Y, sonicPos.Z + 0.5);
